...
public class StickerProvider extends ContentProvider {
 ...
 public boolean onCreate(){try{File d=new File(getContext().getFilesDir(),"stickers");d.mkdirs();File icon=new File(d,"icon.png");if(!icon.exists()){InputStream in=getContext().getAssets().open("icon.b64");BufferedReader r=new BufferedReader(new InputStreamReader(in));StringBuilder s=new StringBuilder();String line;while((line=r.readLine())!=null)s.append(line);r.close();byte[] data=android.util.Base64.decode(s.toString(),android.util.Base64.DEFAULT);FileOutputStream o=new FileOutputStream(icon);o.write(data);o.close();}}catch(Exception ignored){}return true;}
 public String getType(Uri u){return u.getLastPathSegment()!=null&&u.getLastPathSegment().equals("icon.png")?"image/png":"image/webp";}
 public Cursor query(Uri u,String[] p,String s,String[] a,String sort){
  ...
  MatrixCursor c=new MatrixCursor(new String[]{"sticker_file_name","sticker_emoji"});File d=new File(getContext().getFilesDir(),"stickers");File[] fs=d.listFiles();if(fs!=null)for(File f:fs)if(f.getName().endsWith(".webp"))c.addRow(new Object[]{f.getName(),"😀"});return c;
 }
